...
public partial class ScriptManager
{
    ...
    public async Task<T> RunAsync<T>(object? globals)
    {
        ...
        if (compiledScript == null)
        {
            throw new InvalidOperationException("Script compilation failed or has not been completed.");
        }
    ...
    public async Task<ImmutableArray<CompletionItem>> GetCompletionSuggestionsAsync(int position)
    {
        if (document == null)
        {
            throw new InvalidOperationException("Document is not available for code completion. Please compile first.");
        }
    ...
    public async Task<APIInfo.APIInfoResult?> GetSuggestionsAsync(int position)
    {
        ...
        return xmlInfo as APIInfo.APIInfoResult;
    }
    ...
    private static MetadataReference GetMetadataReferenceForRuntime()
    {
        ...
        {
             // Fallback to loaded assembly location if exact file path isn't found
